...
Note: The above example uses ES6 arrow functions. See [functions]() if you need a refresher on either functions or arrow functions.

Note2: In older browsers, NodeLists may not have a `forEach` loop built into them yet. If you encounter this scenario, you can convert your NodeList into an Array with [`Array.from()`]().
...
You may have you can select multiple elements with older methods like `getElementsByClassName` and `getElementsByTagName`. Should you use them?
