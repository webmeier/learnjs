...
For this to work, however, we have to make sure that only the button itself becomes the target of the click event when the mouse is over the button area, not its enclosed `<svg>` nor `<path>` elements. We can prevent those two elements from becoming the target by specifying that they will not respond to pointer events in CSS.
