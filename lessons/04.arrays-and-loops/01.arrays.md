...
Note: `indexOf` only works with primitive values. If you wish to find an object (array, object or function) in the array, you need to use another method like `findIndex`. You'll learn more about `findIndex` in a [later chapter](../07.functional-programming/07.functional-array-methods.md#arrayfindindex).
