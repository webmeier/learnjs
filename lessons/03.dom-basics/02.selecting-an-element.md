...
Try using `id`, `class`, `tag` and `attribute` selectors to get the above four elements. You only need to get each element once. As you do so, notice that you won't be able to get Yoda and Darth Vadar through classes and tags.