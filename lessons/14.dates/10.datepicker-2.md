...
const monthName = monthsInAYear[date.getMonth()].longname
...
Once we have the `monthIndicatorText`, we need to put it into the DOM. To do this, we can select `.datepicker__monthIndicator` and change its text.
...
Notice we had to use `year2` in this code? This is because we already declared and used `year` previously. This is a sign we should refactor our code (and we'll do it later). 
