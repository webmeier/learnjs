# 🛠️ Building a calculator (part 2)
...
Here, we need to check whether the display already contains a decimal point. We can do this with `includes`. `includes` checks whether a string contains another string. If yes, `includes` returns true. If no, it returns `false`. 
...
If the display already contains a decimal point, we do nothing. If the display does not contain a decimal point, we add a `.` to the display. 
...
If Tim hits the decimal key after hitting an operator key, the display should show `0.`.
...
Here we need to know if the previous key is an operator. If it is an operator, we set the display to `0.`. This scenario should come first in the `if` statement. 

(Try reversing the order and see if you can figure out why). 
...
First, if Tim hits an operator key first, the operator key should light up. (Our code covers this case already. But why?).

<figure>
  <img src="../../images/components/calculator/edge/operator-first.gif" alt="Operator key should light up if it's the first key.">
</figure>

Second, nothing should happen if Tim hits the same operator key multiple times. (Our code covers this case already. But why?).
...
Third, if Tim hits another operator key after hitting the first operator key, the first operator key should be released; the second operator key should be depressed. (Our code covers this case too. But why?).
...
First, the display should continue to show 0 if Tim hits the equal key first. 

<figure>
  <img src="../../images/components/calculator/edge/equal-first.gif" alt="Calculator should show zero if equal key is hit first">
</figure>

Second, the display should continue to show the current number if Tim hits the equal key before any operator keys. 
...
For both cases, we know that operator keys have not been clicked yet if `firstValue` is not set to a number. We can use this knowledge to prevent the equal from calculating anything.
...
Third, if Tim hits a number, followed by an operator, followed by a equal, the calculator should calculate the result such that:
...
Our code solves this weird case already. Can you understand why? :)

Fourth, if Tim hits the equal key after a calculation is completed, another calculation should be performed again. Here's how the calculation should read:
...
If the `previousKeyType` is `calculate`, we know we can use `calculator.dataset.modValue` as `secondValue`. Once we know this, we can perform the calculation. (Remember to declare `secondValue` with `let` since we're reassigning a value to it). 
...
Fifth, if Tim hits a decimal key or a number key after the equals key, the display should be replaced with `0.` or the new number respectively.
...
Sixth, if Tim hits an operator key right after the equal key, calculator should NOT calculate.
...
## When Tim hits the clear key... 

If Tim hits the clear key twice (which means he hits `AC`), we need to reset the calculator. Here, we need to reset as well `modValue` (since `modValue` was only introduced in this lesson). 
...
Building a calculator is hard. I made a ton of mistakes when building this! Don't berate yourself if you can't build one without making mistakes. 

For homework: 

1. Write down all the cases I mentioned on a piece of paper 
2. Build the calculator again from scratch without referring to this lesson. 
3. See if you can find any cases I missed :) 

Take your time, clear away your bugs one by one and you'll get your calculator up eventually. In the next lesson, you'll learn to refactor the calculator with best practices.