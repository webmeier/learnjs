...
If you don't understand the problem because you don't know `for` loops, don't worry about it. You will understand the example after you go through `for` loops in detail in a [later chapter](./15.for-loops.md). For now, remember you to declare variables only with `const` or `let`.
