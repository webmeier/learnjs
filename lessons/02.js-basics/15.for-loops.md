...
When the statement has run, JavaScript runs the increment expression and checks if the condition evaluates to `true` again. It repeats this process until the condition evaluates to `false`.
...
(Make sure you know the [basics of arrays](./11.arrays.md) before moving on).
...
Yet another (much better) way to loop through an array to use a `for...of` loop. This is a new loop syntax that comes with ES6.  It looks like this:
...
Another way to loop through an array is with the [Array.forEach](../03.dom-basics/04.foreach.md) method.

## Looping through Objects

If you're reading the `for` loop for the first time, you probably don't need to learn to loop through Objects yet. Don't worry about it for now.
...
If `num` is smaller than 20, you want to add it to another array.

To do so, you first construct an array outside of the loop. Then, whenever you find a number that's smaller than 20, you use `Array.concat` to add the number into the second array.
...
for (let num of numbers) {
  if (num < 20) {
    smallerThan20 = smallerThan20.concat(num)
  }
}
...
- Previous Lesson: [Null and Undefined](14.null-and-undefined.md)
- Next Lesson: [Debugging Errors](16.debugging-errors.md)
