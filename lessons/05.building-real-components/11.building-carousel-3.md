# 🛠 Building a carousel (part 3)

In this lesson, you'll learn to tidy up the carousel code. There'll be no visual changes to the carousel, but the code you write will reduce by about 70%.
...
When you refactor, you try create functions to reuse similar blocks of code. See if you can find better ways to write the code as you do so. It's hard to explain what is "better", but you'll learn about it through the refactoring process.

In this case, we can do both. Let's start by creating functions to reuse code.

## Grouping code into reusable functions

When you reuse code with functions, you write one line of code—the function—as opposed to duplicating lines of code. This is a practice called Don't Repeat Yourself (DRY).
...
This set of code, although understandable, takes up precious brain bandwidth. When we write code, we want to be able to glance through each of our functions and have an idea of what they do.  (Remember, a event listener contains a callback, which is also a function).
...
To get the current slide, you need to return `currentSlide` after the `for` loop. You also want to pass in a `slides` variable, which contains an array of slides in the carousel, to `getCurrentSlide`. As much as possible, we don't want our functions to depend on external variables.
...
Here, we can reuse the code that changes the `is-selected`. We can't reuse the code for `currentDot`, since `targetDot` requires `currentDot` to work.
...
If we assign the correct values to `previousButton`, `nextButton`, and `slides` before our event listeners, we don't need to pass them into the function:
...
Doesn't `getCurrentIndex` look like `getCurrentSlide`? Since they're similar, we can choose between one of them. In this case, let's use `getCurrentIndex` because we can get `currentSlide` from `slides[currentIndex]`. (Why is this so?).
...
One JavaScript best practice is to reduce the number global variables you declare. For the carousel, we created seven variables—`track`, `slides`, `nextButton`, `previousButton`, `dotContainer`, `dots`, and `slideWidth`.

We don't need any of these variables in other components so we should not add these variables to the global scope. Who knows if we'll need a `dots` or `nextButton` variable in another component?

A simple way to prevent yourself from polluting the global scope is to enclose your code within a block scope:
