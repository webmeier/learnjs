...
A tabbed interface is a component that lets you show and hide content. The tabbed interface you're going to build in this lesson looks like this:
...
Make sure you grab the starter files in the Github repo, under `components/04.tabs/01.starter` before moving on.
...
When you clicked on a tab, you want to select that tab. All other tabs should be deselected.

When tabs are not selected, they are not emphasized.
...
For this component, we're going straight for event delegation. You can refer to the lessons on [event delegation](../04.events/06.event-delegation.md) and [building accordions](./06.building-accordions.md) if you need a refresher on the event delegation pattern.

Before continuing further, it makes sense to add JavaScript classes to the element you want to target:

```html
<ul class="tabs jsTabList">
...
One way to find the right tabbed content is through the `href` attribute of the clicked tab. The `href` attribute is only present on anchor tags , which why we looked for `<a>` in the event delegation pattern.

To link the tabs to their respective tabbed content, you need to provide each tab with a `href` that links to an `id` of the tabbed content—tab 1 should have a `href` of `#tab1`, tab 2 should have a `href` of `#tab2` and so on.
