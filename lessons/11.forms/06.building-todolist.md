...
You'll learn how to build a todo list in this lesson. Here's what you'll get when you completed the lesson.
...
First, it is important to recognize that a "todo" can also be called a "task". And "todo" is harder to say compared to "task".

You want to use easier words whenever you can. Your code will become clearer if you do so. This is why we're going to call our todo list a "task list".

## The HTML and CSS for the task list

When you look at the task list component, you will realize that it is divided into two major parts:

1. The part for adding a new task
2. The part that lists the tasks
...
Imagine what a user will do when they finish typing a task in the text field. If the user is adept with keyboards, they may hit the `enter` key to create the new task as well. This behavior is a norm amongst many people who use enough UI.

There is an event that can listen to both the `enter` key and the `click` on the button. This event is a form's `submit` event.

This means the best way to create the "add new task" feature is through a `<form>` element. We also want to set the `autocomplete` attribute to `off` so browsers don't try to autocomplete the fields.
...
But checkboxes are difficult to style. That's why we style the label that comes with a checkbox instead.
...
We want to help our users understand what will happen. To do so, we provide an empty state. In this case, the empty state can be a congratulatory message that says all tasks are completed.
...
We want to make a task with what the user specified. That means we need to pass the task value from the `submit` event listener into `makeTask`.
...
You may notice that the focus gets removed from the text field if user clicks on the plus button to add a task. This prevents the user from adding a second task easily, so we want to bring the focus back onto the text field.
