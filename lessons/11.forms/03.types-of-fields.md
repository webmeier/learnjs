...
(Note: For a list of all available input types, visit [MDN's input reference](https://developer.mozilla.org/en-US/docs/Web/HTML/Element/input))
...
You want to remove unnecessary whitespace from the start and end of the any value you obtain from an text field. To do so, you use the `trim` method that's available for all strings.
...
Checkboxes are often used in tandem with other checkboxes. When you use them this way, make sure you different `name` attributes for each checkbox.
...
console.log(checkedRadio) // <input id="grape" name="grape" ... >
```
