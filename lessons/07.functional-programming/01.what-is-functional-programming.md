...
In Functional Programming, you only care about one thing – **the functions you write must return the same value when the same inputs are given to it.**
...
```js
console.log(addAnotherTen()) // 35
console.log(addAnotherTen()) // 45
console.log(addAnotherTen()) // 55
console.log(addAnotherTen()) // 65
```

**The whole point of Functional Programming is to write functions that doesn't change the state of the program**, like `addTen`.

## State

To define state in programming, it helps to understand the definition of the word, state. The English dictionary defines state as a condition that someone or something is in at a specific time.
...
When `addAnotherTen` runs, JavaScript goes to look for the value of `num`. It finds and changes the current `num` value, within the function. Then, it remembers the new `num` value.
...
When `addTen(orgiNum)` runs, the program assigns the variable `num` with a value of 22. Here, the program is stateful, because it remembers that `num` is 22.
...
In X-men, the genetic makeup of some humans have changed so much that they are able to control new powers. Since all humans are born without power, but they acjuired them because their genes have changed, humans in X-men can be considered mutable.

(That's why X-men are called mutants 😉).

**To envision immutability, think about the computer**that's sitting in front of you right now. It cannot change to something else. It is immutable.
...
Let's say have create a variable called `newEgg` and assign `egg` to it. Then, you decide to change the properties of the original `egg`.
...
When you change `egg`, did you know `newEgg` also gets changed at the same time? This is why mutable objects are scary – you can accidentally change something, and somewhere else, your program gets broken. You'll have no idea what happened and why.
...
When you reassign variables, you change the state of the program. In the following example, `a` is changed from 11 to 100.
...
Don't compare Functional Programming and Object Orientend programming. If you do, you're comparing apples and oranges.

In Function Programming, you want to write pure functions that do not introduce side effects. It should not change the state of the program, either through reassignment or mutation.

If a function is pure, it always returns the same value when the same inputs are given to it. It doesn't rely on the current state of the program. It also doesn't change the state of the program.
