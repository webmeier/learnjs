...
// First things first, create the popover. So, still need the popover code or something
// Second, catch the text within an input event
...
input.addEventListener('input', e => {
  ...
  const matchedName = countries.filter(country =>
    country.name.toLowerCase().includes(currentWord)
  )

  const startsWith = countries.filter(country =>
    country.name.toLowerCase().startsWith(currentWord)
  )

  const matchedCode = countries.filter(country =>
    country.countryCode.toLowerCase() === currentWord
  )

  const combinedList = [...matchedCode, ...startsWith, ...matchedName]
  const displayedList = [...new Set(combinedList)]
    .map(country => country.name)
    .map(name => `<li>${name}</li>`)
    .join('')

  popover.innerHTML = displayedList

  // Note, changing the input value programmatically doesn't fire the 'input' event. Like Google!
})
