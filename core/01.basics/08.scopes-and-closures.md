...
Since `var` variables don't respect block scopes, you sometimes run into an infamous JavaScrip scope problem, where variables aren't what you'd expect them to be.
...
<figure>
  <img src="../../images/core/01/scopes/block-var.png" alt="var variables don't respect block scopes">
...
<figure>
  <img src="../../images/core/01/scopes/block-let.png" alt="const and let variables respect block scopes">
...
If you don't understand the problem because you don't know `for` loops, don't worry about it. You will understand the example after you go through `for` loops in detail in a [later chapter](). For now, remember you to declare variables only with `const` or `let`.
...
<figure>
  <img src="../../images/core/01/scopes/one-way-glass.png" alt="Scopes in functions behave like a one-way-glass. You can see the outside, but people outside can't see you">
...
<figure>
  <img src="../../images/core/01/scopes/glass-layers.png" alt="Multiple layers of functions mean multiple layers of one-way glass">
...
If you're just starting out, you'd almost never write closures. They're for more advanced applications (like [controlling side effects]() (not done yet) and [creating private variables]() (not done yet)).

Even though you might not use closures at this point, it's important to understand what they are, so you don't get confused by tutorials that love the *closure* jargon.

Closures are simple.

When you create an `innerFn` in an `outerFn`, you have created a closure. The `innerFn` is the closure.
...
When we work with closures, we usually return the `innerFn`, so we can use its variables at a later time.

As you can see in the example below, `innerFn` has access to `outerVar` even though we call it outside `outerFn`.

```js
function outerFn () {
  var outerVar = `I'm the outer function!`
  return function innerFn () {
...
We'll talk more about closures and their application in the lesson on [controlling side effects]() (not done yet).

## Debugging scopes with devtools

Chrome's (and Firefox's) developer tools make it simple for you to scopes. There are two ways to use this functionality.
...
<figure>
  <img src="../../images/core/01/scopes/debugger1.png" alt="Debugging prepareCake's scope">
  <figcaption>Debugging outerFn's scope</figcaption>
</figure>

In the image above, you can see that you can access `outerVar` and `innerFn`.

Try shifting the `debugger` keyword into the closure (`innerFn`) and notice how the scope changes:
...
<figure>
  <img src="../../images/core/01/scopes/debugger2.png" alt="Debugging the innerFn scope">
  <figcaption>Debugging the innerFn scope</figcaption>
</figure>

You can see that `innerFn` has access to `innerVar`.

Now, add a `console.log` statement for `outerVar` within `innerVar` and you can see the `outerVar` is in a closure scope:
...
<figure>
  <img src="../../images/core/01/scopes/debugger3.png" alt="Getting outerVar in innerFn">
  <figcaption>Getting outerVar in innerFn</figcaption>
</figure>

The **second way** use the the same `debugger` functionality is to add a breakpoint to your code directly in the Sources (or Debugger) tab by clicking on the line number.

After you add the breakpoint, refresh your page and JavaScript will pause *just before executing your breakpoint*.

<figure>
  <img src="../../images/core/01/scopes/breakpoint.png" alt="Debugging scopes by adding breakpoints">
...
4. Create a closure.
5. Get the value of a closure after the outerFn is called
