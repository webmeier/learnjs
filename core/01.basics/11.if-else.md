...
And what happens when the light turns green again? You start walking, don't you?
...
Well, let's say you want to cross a small road. There aren't any cars around. Would you wait for the traffic light to turn green? You still cross, don't you? 😉
...
However, there's a difference between strictly equal (`===`) and equal (`==`):
...
Controlling the flow of a program with `if/else` statements is incredibly important when you start out. Try the following exercises:

1. Make an `if/else` statement.
2. Make a `else if` statement within your `if/else`.
3. Compare two values with `>` and `>=`
4. Compare two values with `<` and `<=`
5. Compare two values with `===`
6. Compare two values with `!==`
